...
struct LissageMedianGrappe : public Worker
{
  ...
  void operator()(std::size_t begin, std::size_t end)
  {
    ...
    const unsigned int iMaxLigne = *max_element(std::begin(vLigneObservations), std::end(vLigneObservations)) + 1;
    const unsigned int iMaxColonne = *max_element(std::begin(vColonneObservations), std::end(vColonneObservations)) + 1;
    ...
    for(std::size_t i = begin; i < end; ++i)
    {
      ...
      vector<int> vCoordGrappe = coordonneesGrappe(std::log2(iTailleMatrice), iNoGrappe);
